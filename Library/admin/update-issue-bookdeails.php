...
$sql = "SELECT students.FullName,books.BookName,books.id,issuedbookdetails.IssuesDate,issuedbookdetails.series,issuedbookdetails.ReturnDate,issuedbookdetails.id as rid,issuedbookdetails.fine,issuedbookdetails.RetrunStatus,issuedbookdetails.finePay from  issuedbookdetails join students on students.StudentId=issuedbookdetails.StudentId join books on books.id=issuedbookdetails.BookId where issuedbookdetails.id=:rid";
$query = $dbh -> prepare($sql);
$query->bindParam(':rid',$rid,PDO::PARAM_STR);
$query->execute();
...
if($query->rowCount() > 0)
{
    ...
    {?> 
        ...
        <?php echo htmlentities($result->BookName);?>
                                ...
                                <div class="form-group">
                                    <label>ISBN :</label>
        <?php echo htmlentities($result->id);?>/<?php echo htmlentities($result->series);?>
                                </div>
                                <div class="form-group">
